...
module.exports = class ZecoreProductHelper{
    ...
    async RegistrarProducto(){
        ...
        try{
            const result = await db.execute('insert into producto(idProducto, FK_idMarca_producto, Nombre, Imagen, Descripcion, Categoria) values(?,?,?,?,?,?)',
        [this.idProducto,this.marcaProducto,this.Nombre,this.Imagen,this.Descripcion,this.categoria]);
    ...
    try {
        const result = await db.execute(`UPDATE producto SET ${columna} = ? WHERE idProducto = ?`,[nuevoValor,id])
